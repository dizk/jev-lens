...
console.log("| condition | runs | passed | calls | uncached input | cached input | cache hit | output | wall (s) | pruned tokens |");
console.log("|---|---|---|---|---|---|---|---|---|---|");
for (const c of conds) {
	...
	console.log(`| ${c} | ${rs.length} | ${rs.filter((r) => r.ok).length} | ${sum((r) => r.calls)} | ${k(input)} | ${k(cached)} | ${pct(cached / Math.max(1, input + cached))} | ${k(sum((r) => r.output))} | ${Math.round(sum((r) => r.wallMs) / 1000)} | ${k(sum((r) => r.pruned ?? 0))} |`);
}
