...
beforeEach(() => {
	vi.stubEnv("JEV_MEMORY_CLASSIFIER", "mock");
	vi.stubEnv("JEV_MEMORY_VARIANT", "");
	vi.stubEnv("JEV_MEMORY_UI", "0");
	vi.stubEnv("JEV_MEMORY_LOG", "0");
	vi.stubEnv("JEV_MEMORY_CLASSIFY_WAIT_MS", "30");
});
afterEach(() => {
...
describe("asynchronous session isolation and durable flush", () => {
	it("discards old responses without deleting new in-flight work with the same id", async () => {
		...
		expect(existsSync(join(h.ctx.cwd, ".pi/jev-memory.md"))).toBe(false);
	...
	it("discards text notes from an earlier session", async () => {
		vi.stubEnv("JEV_MEMORY_CLASSIFIER", "real"); vi.stubEnv("TYPESAFE_API_KEY", "test-key");
		...
		expect(existsSync(join(next.cwd, ".pi/jev-memory.md"))).toBe(false);
	});
	it("flushes notes arriving after the agent-end wait expires", async () => {
		...
		await new Promise((r) => setTimeout(r, 0));
		expect(readFileSync(join(h.ctx.cwd, ".pi/jev-memory.md"), "utf8")).toContain("succeeded");
	});
	...
	it("tracks text requests and flushes them before shutdown", async () => {
		...
		expect(readFileSync(join(h.ctx.cwd, ".pi/jev-memory.md"), "utf8")).toContain("Always preserve");
	});
