...
## 2026-09-19: monorepo and a Claude Code host

The repository became `dizk/jev-lens` with three packages: `jev-lens` (the core: views, tree-sitter, the jev questions, the decision rule, expansion, recall slicing, behind `Lens.compress`), `pi-jev-lens` (the pi extension, now a host over the core) and a Claude Code plugin (`packages/claude-code`). Nothing about views, prompts or thresholds changed; the 145 existing tests pass unchanged after the move, plus 15 new ones for the plugin.

The Claude Code host is a PostToolUse hook returning `updatedToolOutput` (Claude Code 2.1.278 docs; it works for built-in tools and must match the tool's output shape) and a dependency-free MCP server with `recall` and `stats`. Verified by hand: a 400-line pytest log (1909 tokens) became the testlog view (256 tokens) in 0.93 s wall including node startup and two jev requests; a code read with no task context stayed full (needs-full 0.29, full mass 0.43, gate). What is different from pi and not yet measured: the agent's text before the call comes from the transcript file, which can lag the current turn; Read results get Claude Code's own sequential line numbers in front of the view's, so the footer explains that; there is no post-send layer. No Claude Code sessions have been replayed yet; the log in `~/.claude/jev-lens/log.jsonl` (or the plugin data directory) records the same fields as pi's, so the replay harness can be pointed at Claude Code transcripts later.
