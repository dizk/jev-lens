...
- `/jev-context key` stores the TypeSafe API key in `~/.pi/agent/jev-context.json` (user-only); `TYPESAFE_API_KEY` in the environment takes precedence.
- Cache-aware post-send pruning ledger (`budget` mode by default) and durable notes in `.pi/jev-context.md`.
